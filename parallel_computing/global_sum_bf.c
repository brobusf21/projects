 ...
 *
 * Purpose:  Implements a butterfly-structured global sum starting from the
 *           most distant processes paired first and the adjacent processes
 *           paired last.  
 * 
 * Input:    None.
 * Output:   Random values generated by processes, and sum of random values.
 *
 * Compile:  mpicc -g -Wall -o global_sum_bf global_sum_bf.c
 * Run:      mpiexec -n <number of processes> ./global_sum_bf
 *
 * Notes:     
 *    1.  p, the number of processes, *must* be a power of 2.
 *    2.  The result returned by all the processes is valid.
 *    3.  This program was supplied by Prof. Pacheco and I have modified 
 *        it according to the assignment requirements. 
 */
#include <stdio.h>
#include <stdlib.h>
#include <mpi.h>
 ...
 *              Distant processes are paired first and adjacent processes
 *              are paired last.
 *
...
int Global_sum(int my_contrib, int my_rank, int p, MPI_Comm comm) {
    ...
    printf("Process %d enters with no issues so far", my_rank);
