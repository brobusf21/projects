 ...
 *           and a random vector. This version has been modified to 
 *           include the timings for the matrix-vector multiplication. 
 *
 * Input:
 *     m, n: order of matrix
 *
 ...
 *     3.  MPI_Wtime() is used to start and finsish the timings.
 *     4. Get_max_time function calculates the max timing
 *
 */
...
void Read_vector(char* prompt, float local_x[], int local_n, int my_rank,
             int p, MPI_Comm comm);
double Parallel_matrix_vector_prod(float local_A[], int m, 
...
int main(int argc, char* argv[]) {
    ...
    int             m, n;
    int             local_m, local_n;
    double          par_elapsed, total, max_par_elapsed;
    MPI_Comm        comm;
...
//  Print_vector("We read", local_x, local_n, my_rank, p, comm);
    ...
    par_elapsed = Parallel_matrix_vector_prod(local_A, m, n, local_x, global_x, 
        local_y, local_m, local_n, comm);
    
    /* What is the max time for the processes */
    max_par_elapsed = Get_max_time(par_elapsed, my_rank, p);
 ...
 */
double Parallel_matrix_vector_prod(
         float    local_A[]   /* in  */,
         int      m           /* in  */,
         int      n           /* in  */,
         float    local_x[]   /* in  */,
